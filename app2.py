...
class TimestampLoggerApp:
    def __init__(self, master):
        ...
        if self.experiment_name:
            ...
            self.log_button = tk.Button(master, text="Log beginning  Timestamp", command=self.log_current_timestamp)
            self.log_button.pack(pady=10)
            self.end_button  = tk.Button(master,text= " Log  end  of freezing ",command =  self.log_current_timestamp_end)
    ...
    def log_current_timestamp(self):
        if self.start_time is not None:
            ...
            with open(self.log_file_name, 'a') as f:
                f.write(f"Start of freeze : {current_time}\n")
            print(f"Current timestamp logged: {current_time}")
        else:
            print("Please start the logging first.")


    def log_current_timestamp_end(self):
        if self.start_time is not None:
            current_time = datetime.now()
            with open(self.log_file_name, 'a') as f:
                f.write(f"End of freeze log  timestamp: {current_time}\n")
            print(f"Current timestamp logged: {current_time}")
        else:
            print("Please start the logging first.")
